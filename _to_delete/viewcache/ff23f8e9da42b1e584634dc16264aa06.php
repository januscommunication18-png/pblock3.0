<?php $__env->startSection('title', 'Onboarding · Profile — Project Block'); ?>
...
<?php $__env->startSection('body'); ?>
  ...
  <main class="flex-1 flex justify-center px-5">
    <div class="w-full max-w-[430px] py-6 sm:py-12">
      ...
      <?php if($errors->any()): ?>
        <div class="mb-4 rounded-lg border border-danger/40 bg-danger/5 px-3.5 py-2.5 text-[13px] text-danger"><?php echo e($errors->first()); ?></div>
      <?php endif; ?>
      ...
      <div class="flex items-center gap-4 mb-5">
        <span id="avatar" class="h-14 w-14 rounded-full bg-brand grid place-items-center text-white text-[20px] font-semibold bg-cover bg-center shrink-0"
          <?php if($user->avatar_url): ?> style="background-image:url('<?php echo e($user->avatar_url); ?>')" <?php endif; ?>><?php echo e($user->avatar_url ? '' : $user->initial()); ?></span>
      ...
      <form method="POST" action="<?php echo e(route('onboarding.profile.store')); ?>">
        ...
        <input id="fullname" name="full_name" type="text" value="<?php echo e(old('full_name', $user->full_name)); ?>" placeholder="Enter your full name" class="pb-input <?php echo e($errors->has('full_name') ? 'is-error' : ''); ?>" />
        ...
        <div class="mt-4 border border-line rounded-lg">
          <button id="pw-toggle" type="button" class="w-full flex items-center gap-2 h-11 px-3.5 text-[14px] text-sub">
            ...
            <svg id="pw-chevron" width="16" height="16" viewBox="0 0 24 24" fill="none" class="ml-auto transition-transform <?php echo e($errors->has('password') ? 'rotate-180' : ''); ?>"><path d="M6 9l6 6 6-6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
          </button>
          <div id="pw-body" class="<?php echo e($errors->has('password') ? '' : 'hidden'); ?> px-3.5 pb-3.5 space-y-3">
            <input type="hidden" name="set_password" id="set_password" value="<?php echo e(old('set_password', $errors->has('password') ? 1 : 0)); ?>" />
            <div>
              <label class="block text-[13px] text-sub mb-1.5">Set a password</label>
              <div class="relative">
                <input name="password" type="password" placeholder="Set a password" class="pw-input pb-input has-suffix <?php echo e($errors->has('password') ? 'is-error' : ''); ?>" />
...
$message = $__bag->first($__errorArgs[0]); ?><p class="mt-1 text-[12px] text-danger"><?php echo e($message); ?></p><?php unset($message);
...
unset($__errorArgs, $__bag); ?>
        ...
        <label class="mt-5 flex items-start gap-2.5 text-[13px] text-ink cursor-pointer">
          <input type="checkbox" name="marketing_opt_in" value="1" <?php echo e(old('marketing_opt_in', $user->marketing_opt_in) ? 'checked' : ''); ?> class="mt-0.5 h-4 w-4 accent-[#1b5f8a]" />
  ...
  <script>
    ...
    (function () {
      ...
      input.addEventListener('change', function () {
        ...
        xhr.open('POST', '<?php echo e(route('onboarding.avatar.store')); ?>');
...
<?php echo $__env->make('layouts.auth', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/rohitphilip/Sites/pblock3.0/resources/views/onboarding/profile.blade.php ENDPATH**/ ?>