<?php $__env->startSection('title', 'Projects — Project Block'); ?>

<?php $__env->startSection('body'); ?>
  
  <form method="POST" action="<?php echo e(route('logout')); ?>" id="logout-form" class="hidden"><?php echo csrf_field(); ?></form>
  ...
  <header class="h-14 shrink-0 border-b border-line flex items-center gap-2 px-3 sm:px-4">
    ...
    <div class="flex items-center gap-2 shrink-0">
      <span class="flex items-center gap-2 px-2 h-9">
        ...
        <span class="font-medium text-[13px] max-w-[110px] sm:max-w-[150px] truncate"><?php echo e($workspace->name); ?></span>
      </span>
    ...
    <div class="flex items-center gap-1.5 shrink-0">
      <a href="<?php echo e(route('welcome')); ?>" class="hidden sm:inline-flex items-center h-8 px-3 rounded-md border border-line text-[13px] text-ink hover:bg-hover">Get started</a>
      ...
      <div class="relative ml-1">
        <button id="user-btn" class="h-7 w-7 rounded-full bg-emerald-500 grid place-items-center text-white text-[11px] font-bold ring-2 ring-transparent focus:outline-none"><?php echo e($user->initial()); ?></button>
        <div id="user-menu" class="hidden absolute right-0 top-full mt-2 w-64 bg-white border border-line rounded-xl shadow-lg z-50 p-1.5">
          <div class="relative rounded-lg overflow-hidden px-4 pt-7 pb-4 text-center" style="background-color:#9ca3af;">
            <span class="h-14 w-14 rounded-full bg-brand text-white grid place-items-center text-[20px] font-semibold mx-auto shadow-sm"><?php echo e($user->initial()); ?></span>
            <div class="text-[14px] font-semibold text-white mt-2.5 drop-shadow-sm"><?php echo e($user->displayName()); ?></div>
            <div class="text-[12px] text-white/90 drop-shadow-sm"><?php echo e($user->email); ?></div>
          </div>
          <div class="pt-1.5">
            <a href="<?php echo e(route('workspaces.create')); ?>" class="w-full text-left flex items-center gap-2.5 px-2.5 h-9 rounded-lg text-[13px] text-ink hover:bg-hover">
            ...
            <a href="<?php echo e(route('settings.general')); ?>" class="w-full text-left flex items-center gap-2.5 px-2.5 h-9 rounded-lg text-[13px] text-ink hover:bg-hover">
    ...
    <div class="relative w-full max-w-[860px] bg-white rounded-xl shadow-xl flex flex-col max-h-[88vh]">
      ...
      <div class="px-5 sm:px-6 pt-5 pb-1 overflow-visible">
        <div class="flex flex-col sm:flex-row gap-3">
          <div class="flex-1">
            <input id="proj-name" type="text" placeholder="Project name" class="pb-input w-full" maxlength="<?php echo e($config['nameMax']); ?>" />
            <p data-err="name" class="hidden text-[12px] text-red-600 mt-1"></p>
          </div>
          <div class="relative sm:w-44">
            <input id="proj-id" type="text" placeholder="Project ID" class="pb-input pr-9 uppercase w-full" maxlength="<?php echo e($config['identifierMax']); ?>" />
        ...
        <textarea id="proj-desc" rows="3" placeholder="Description" class="pb-textarea mt-3 w-full" maxlength="<?php echo e($config['descriptionMax']); ?>"></textarea>
  ...
  <div id="ws-modal" class="hidden fixed inset-0 z-[60] flex items-start justify-center p-4 sm:pt-24">
    ...
    <div class="relative w-full max-w-[560px] bg-white rounded-xl shadow-xl flex flex-col max-h-[80vh]">
      ...
      <div id="ws-modal-list" class="overflow-y-auto px-4 py-3 space-y-2" data-workspaces='<?php echo json_encode($workspaces, 15, 512) ?>'></div>
      <div class="px-6 py-4 border-t border-line shrink-0">
        <a href="<?php echo e(route('workspaces.create')); ?>" class="w-full inline-flex items-center justify-center gap-2 h-9 rounded-md border border-stroke text-[13px] font-semibold text-ink hover:bg-hover">
    ...
    <div class="fixed top-4 right-4 z-[80] w-full max-w-sm flex justify-end pointer-events-none">
      <div id="toast" class="pointer-events-auto w-full max-w-sm overflow-hidden rounded-lg bg-white shadow-lg ring-1 ring-black/5">
        <div class="p-4"><div class="flex items-start">
          ...
          <div class="ml-3 w-0 flex-1 pt-0.5"><p class="text-[13px] font-semibold text-head">Success</p><p class="mt-1 text-[13px] text-sub"><?php echo e(session('status')); ?></p></div>
  ...
  <script id="pb-projects-data"
          ...
          data-projects='<?php echo json_encode($projects, 15, 512) ?>'
          data-members='<?php echo json_encode($members, 15, 512) ?>'
          data-config='<?php echo json_encode($config, 15, 512) ?>'
          data-endpoints='<?php echo json_encode($endpoints, 15, 512) ?>'></script>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/rohitphilip/Sites/pblock3.0/resources/views/app/projects.blade.php ENDPATH**/ ?>