...
<head>
  ...
  <title>Epics — {{ $project->name }}</title>
  ...
  {{-- An epic's work items are the SAME grid as the project's work item list. --}}
  @include('partials.work-item-assets')

  {{-- The Epic's Work Items tab mounts the WORK ITEMS SCREEN itself, not a copy of it — so
       chips are editable and a row opens the same drawer. This file defines the component;
       its own boot is guarded by data-screen, so it does not mount over the epics root. --}}
  ...
  <script defer src="{{ pb_asset('assets/js/projects/epics.js') }}"></script>
</head>
  ...
  <div class="flex-1 flex min-h-0 relative">
    ...
    <main class="flex-1 min-w-0 flex flex-col overflow-hidden">
      ...
      <div id="settings-root" class="flex-1 min-h-0 flex flex-col" data-bootstrap='@json($bootstrap)'>
        <div class="px-6 py-10 text-sub text-[13px]">Loading epics…</div>
      </div>
    </main>
  </div>
</body>
