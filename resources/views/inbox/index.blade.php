...
<head>
  ...
  <title>Inbox — {{ $workspace->name }}</title>
  ...
  {{-- The row vocabulary: the same avatars and state glyphs every other list draws with. --}}
  @include('partials.work-item-chips')
  <script defer src="{{ pb_asset('assets/js/inbox.js') }}"></script>
</head>
  ...
  <div class="flex-1 flex min-h-0 relative">
    ...
    <main class="flex-1 min-w-0 flex flex-col overflow-hidden">
      <div id="settings-root" class="flex-1 min-h-0 flex flex-col" data-bootstrap='@json($bootstrap)'>
        <div class="px-6 py-10 text-sub text-[13px]">Loading your inbox…</div>
      </div>
    </main>
  </div>
</body>
