...
| V3 | Filters, sorts, groups (§12.2–12.4, `project_view_filters`, `project_view_sorts`) | **Deferred to Slice 2.** Slice 1 ships toolbar Search and per-column sort. Saved multi-condition filters are the same infrastructure the Work Items, Epic, Estimation and Labels screens have each deferred waiting for, so it gets designed once, deliberately, rather than being grown here. |
