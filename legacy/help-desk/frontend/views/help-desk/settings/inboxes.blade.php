...
<head>
  ...
  <title>Inboxes — Help Desk — {{ $workspace->name }}</title>
  ...
  <script defer src="{{ pb_asset('assets/js/help-desk/inboxes.js') }}"></script>
</head>
    ...
    <main class="flex-1 min-w-0 flex flex-col overflow-hidden">
      <div class="h-12 shrink-0 border-b border-line flex items-center gap-3 px-5 sm:px-8">
        ...
        <nav class="ml-4 flex items-center gap-1">
          ...
          <a href="{{ route('help-desk.activity') }}" class="h-7 px-3 grid place-items-center rounded-md text-sub hover:bg-hover text-[12px]">Activity</a>
        </nav>
      </div>

      <div class="flex-1 min-h-0 overflow-y-auto">
        <div id="help-desk-inboxes-root" data-bootstrap="{{ json_encode($bootstrap) }}">
