 ...
 * controller, so the invite-acceptance path lands on the same rules instead of a second,
 * similar set of them.
 *
 * Each change is recorded in the activity stream from HERE for the same reason: the screen, the
 * wizard and an accepted invitation all arrive through these methods, and an audit trail
 * written by the callers would be missing whichever caller forgot (FR-1.9, §8).
 */
class HelpDeskMemberManager
{
    ...
    public function add(HelpDesk $helpDesk, User $actor, int $userId, string $role, array $inboxIds = []): HelpDeskMember
    {
        ...
        return DB::transaction(function () use ($helpDesk, $actor, $userId, $role, $inboxIds) {
            ...
            $this->setInboxes($helpDesk, $member, $inboxIds, record: false);

            $member = $member->fresh();
            $this->activity->memberAdded($helpDesk, $actor, $member);
    ...
    public function updateRole(HelpDesk $helpDesk, HelpDeskMember $member, string $role, ?User $actor = null): HelpDeskMember
    {
        $this->assertRole($role);
        ...
        $member = $member->fresh();
        $this->activity->roleChanged($helpDesk, $actor, $member, $from);
     ...
     *
     * `$record` is false only when the caller has already recorded the bigger event this is
     * part of — adding somebody names their inboxes as part of adding them, and two entries for
     * one action is a stream that has to be read twice to learn one thing.
     *
     * @param  array<int, int>  $inboxIds
     */
    public function setInboxes(
        HelpDesk $helpDesk,
        HelpDeskMember $member,
        array $inboxIds,
        ?User $actor = null,
        bool $record = true,
    ): HelpDeskMember {
        ...
        $named = HelpDeskInbox::query()
            ->where('help_desk_id', $helpDesk->id)
            ->whereIn('id', array_map('intval', $inboxIds))
            ->pluck('name', 'id');

        $member->inboxes()->sync($named->keys()->all());

        $member = $member->fresh();

        if ($record) {
            $this->activity->inboxesChanged($helpDesk, $actor, $member, $named->values()->all());
        }
     ...
     */
    public function setStatus(HelpDesk $helpDesk, HelpDeskMember $member, string $status, ?User $actor = null): HelpDeskMember
    {
        ...
        $member = $member->fresh();
        $this->activity->statusChanged($helpDesk, $actor, $member);
     ...
     */
    public function remove(HelpDesk $helpDesk, HelpDeskMember $member, ?User $actor = null): void
    {
        // Recorded BEFORE the delete, while the member can still be described.
        $this->activity->memberRemoved($helpDesk, $actor, $member);
