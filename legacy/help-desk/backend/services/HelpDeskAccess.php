...
class HelpDeskAccess
{
    ...
    /**
     * The spaces this person may work in (Workspace & Inbox Assignment §15).
     *
     * DERIVED from inbox access rather than granted separately (decision H38). §15 suggests a
     * second set of roles per space, but this Help Desk already decides who reaches what at the
     * inbox level (FR-1.7), and a space is a group of inboxes — so "which spaces may I see?" has
     * an answer already: the ones holding an inbox I can open. A third access layer would be a
     * third thing to keep in step with the other two, and §5's most-restrictive-wins rule would
     * have to be re-derived every time they disagreed.
     *
     * Admins and Managers reach every inbox, so they get every space — including empty ones,
     * which they are the only people who can do anything about.
     *
     * Archived spaces are never here: §8 archives a space to take it out of the way, and a
     * switcher that still offers it has not taken it anywhere.
     *
     * @return array<int, array<string, mixed>>
     */
    public function visibleSpaces(?User $user, ?Workspace $workspace): array
    {
        if (! $this->canOpen($user, $workspace)) {
            return [];
        }
        ...
        return HelpDeskSpace::query()
            ...
            // A list — including an empty one — is exactly those inboxes; null is every inbox,
            // and then every space qualifies whether or not it holds one.
            ->when($allowed !== null, fn ($q) => $q->whereHas(
                'inboxes',
                fn ($i) => $i->withoutGlobalScopes()->whereIn('help_desk_inboxes.id', $allowed ?: [0]),
            ))
            ->orderBy('name')
            ->get()
            ->map(fn (HelpDeskSpace $space) => [
                'id' => $space->id,
                'name' => $space->name,
    ...
    /**
     * May this person open that specific space?
     *
     * Administrators are included whether or not they hold an inbox in it: configuring a space
     * they cannot reach operationally is exactly what H6 gives a workspace owner.
     */
    public function canOpenSpace(?User $user, ?Workspace $workspace, int $spaceId): bool
    {
        if ($this->canAdminister($user, $workspace)) {
            return true;
        }

        foreach ($this->visibleSpaces($user, $workspace) as $space) {
            if ((int) $space['id'] === $spaceId) {
                return true;
            }
        }
