...
class EnableHelpDeskTest extends HelpDeskTestCase
{
    ...
    private function enabled(Workspace $workspace): bool
    {
    ...
    public function test_the_create_screen_offers_help_desk_as_a_real_choice(): void
    {
        ...
        $newcomer = User::factory()->create(['current_workspace_id' => null]);
    ...
    public function test_switching_the_app_on_or_off_is_logged(): void
    {
        [, $workspace] = $this->owner('acme-inc', ['projects']);

        // §13: log security-sensitive configuration changes. Whether a whole area of the
        // product exists for a workspace is one.
        Log::shouldReceive('info')
            ->once()
            ->with('workspace.app.enabled', \Mockery::on(
                fn (array $context) => $context['app'] === 'helpdesk' && $context['workspace_id'] === $workspace->id
            ));

        $this->enable($workspace);
    }

    public function test_saving_the_same_app_list_again_logs_nothing(): void
    {
        [, $workspace] = $this->owner();

        // A log that records non-changes says nothing while burying the entries that mean
        // something — and Settings → General resends this list on every save.
    ...
    public function test_the_area_opens_when_enabled(): void
    {
        ...
        // Past the wizard — a Help Desk nobody has set up yet sends its administrator there
        // instead of to the overview (FR-1.3, covered in SetupWizardTest).
