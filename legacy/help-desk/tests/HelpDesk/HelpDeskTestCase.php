...
abstract class HelpDeskTestCase extends TestCase
{
     ...
     *
     * Marked as SET UP by default: a Help Desk that has never been through the wizard sends its
     * administrator there (FR-1.3), and almost every test here is about what happens afterwards.
     * Pass `$setUp: false` to test the wizard itself.
     */
    protected function helpDesk(Workspace $workspace, ?User $creator = null, bool $setUp = true): HelpDesk
    {
        return $workspace->run(function () use ($workspace, $creator, $setUp) {
            $helpDesk = app(HelpDeskProvisioner::class)->for($workspace, $creator);

            if ($setUp && $helpDesk->setup_completed_at === null) {
    ...
    protected function addToHelpDesk(
        ...
        $helpDesk = $this->helpDesk($workspace, $actor, $setUp);
